...
export const embedsSchema = z.object({
	...
	image: z
		.object({ url: z.string().url('Please enter a valid image URL for the embed image') })
		.optional(),
	fields: fieldsSchema
		.array()
		.max(25, 'Fields length must be less than 25 fields per embed')
		.optional(),
	footer: z
		.object({
		...
		.optional(),
	thumbnail: z
		.object({ url: z.string().url('Please enter a valid image URL for the embed thumbnail') })
		.optional(),
...
export const hookJsonPartial = z
	.object({
		username: z
			.string()
			.trim()
			.max(80, 'Username length must be less than 80 characters')
			.optional(),
		...
		thread_name: z
			.string()
			.max(100, 'Thread name length must be less than 100 characters')
			.optional()
	})
	.partial();
...
export const fieldsFullyPartialSchema = z.object({
	...
	value: z
		.string()
		.max(1024, 'Embed field value length must be less than 1024 characters')
		.optional(),
	inline: z.boolean().optional()
});
...
export const embedsFullyPartialSchema = z.object({
	...
	description: z
		.string()
		.max(4096, 'Description length must be less than 4096 characters')
		.optional(),
	color: z.number().optional(),
	url: z.string().trim().optional(),
	...
	fields: fieldsFullyPartialSchema
		.array()
		.max(25, 'Fields length must be less than 25 fields per embed')
		.optional(),
	footer: z
...
export const hookJsonFullyPartialSchema = z
	.object({
		...
		content: z.string().max(2000, 'Content length must be less than 2,000 characters').optional(),
		embeds: embedsFullyPartialSchema
			.array()
			.max(10, 'Embeds length must be less than 10 items')
			.optional(), //.max(10, "Embeds length must be less than 10 items")
		files: filesSchema.optional(),
		thread_name: z
			.string()
			.max(100, 'Thread name length must be less than 100 characters')

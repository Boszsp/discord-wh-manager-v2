...
import {
	...
	type hookJsonFullyPartialSchemaType,
	type hookJsonPartialSchemaType
} from '$lib/schema/webhookContentSchema';
import { consola } from 'consola';

export function safePareseTemplateString(
	str: string,
	retureOnErrorValue?: hookJsonPartialSchemaType
) {
	try {
		const { data, success, error } = hookJsonFullyPartialSchema.safeParse(JSON.parse(str));
		if (success) return retureOnErrorValue ? Object.assign(retureOnErrorValue, data) : data;
		else {
...
export function safeStrinifyTemplateString(obj: hookJsonFullyPartialSchemaType) {
	try {
		...
		if (success) return data || DEFAULT_WEBHOOK_CONTENT_AS_STRING;
		else {
			consola.error('Error parsing template string:', error);

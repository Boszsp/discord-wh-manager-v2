...
export async function createPdfFromImages(
	...
	for (let i = 0; i < images.length; i++) {
		...
		try {
			...
			try {
				...
				console.error(
					`Could not embed image: ${imageFile.name}. It might be an unsupported format.`,
					e
				);
	...
	const fileName =
		name && name?.length > 1
			? name + '.pdf'
			: images?.[0].name?.replaceAll('/', '-').replaceAll('.', '-') + '.pdf';
...
export async function extractPdfImages(pdf: File, fileName?: string): Promise<File[]> {
	...
	for (let pageNum = 1; pageNum <= pdfDocument.numPages; pageNum++) {
		...
		if (capturedBitmaps.length !== imageObjectIds.length) {
			console.warn(
				`Page ${pageNum}: Mismatch between captured images (${capturedBitmaps.length}) and found IDs (${imageObjectIds.length}).`
			);
		}

    ...
    for (let i = 0; i < pageCount; i++) {
        ...
        if (currentSize > maxSizeInBytes) {
            ...
            if (pagesToCopy.length > 0) {
                ...
                pdfs.push(new File([savedBytes], `${file.name.replace('.pdf', '')}-part${part++}.pdf`, { type: 'application/pdf' }));
            }
...
async function splitZip(
    ...
    for (const fileName in unzipped) {
        ...
        if (currentSize + fileData.length > maxSizeInBytes && Object.keys(currentZipFiles).length > 0) {
            ...
            zips.push(new File([zipBlob], `${file.name.replace('.zip', '')}-part${part++}.zip`, { type: 'application/zip' }));
            currentZipFiles = {};
            currentSize = 0;
        }

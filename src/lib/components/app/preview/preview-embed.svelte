...
<Card class="relative grid w-full max-w-md gap-2 overflow-hidden rounded-sm p-4 text-sm">
	<div
		...
		<span>
			<div>
				{#if embed?.author?.name}
					<div class="flex items-center gap-2">
						...
						<div class="text-sm font-bold">
							{#if embed?.author.url && embed?.author.url?.length > 1}
				...
				{#if embed?.title || embed?.description}
					<div class="mt-2">
						{#if embed?.title}
							{#if embed?.url && embed?.url?.length > 0}
